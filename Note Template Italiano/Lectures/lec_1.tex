...
\begin{definizione}[Natural number]\label{def}
...
\begin{lemma}[Useful lemma]\label{lma}
  ...
  \[
    0\neq 1.
  \]
  \begin{dimostrazione}[An obvious proof]
    Obvious.
  \end{dimostrazione}
\end{lemma}
\begin{proposizione}[Useful proposition]\label{prop}
...
\begin{teorema}[Mass-energy equivalence]\label{thm}
...
\begin{corollario}[Riemann hypothesis]\label{col}
...
\begin{definizione*}
  ...
  \begin{definizione}[Rational number]\label{def:rational}
    The set of \emph{rational number}, denote as \(\mathbb{\MakeUppercase{q}} \).
  \end{definizione}
  \begin{definizione}[Real number]\label{def:real}
    The set of \emph{real number}, denote as \(\mathbb{\MakeUppercase{r}} \).
  \end{definizione}
  \begin{definizione}[Complex number]\label{def:complex}
...
\begin{es}[Interesting Example]\label{eg}
  We note that \(1 \neq 2\)!
  \begin{nota}[Important note]
    As a consequence, \(2 \neq 3\) also.
  \end{nota}

  \begin{osservazione}[Easy observation]
    We see that from here, we easily have the following theorem.
    \begin{teorema}[Lebesgue Differentiation Theorem]\label{thm:Lebesgue-differentiation-theorem}
      Let \(f\in L^1\), then
      \[
