...
\begin{itemize}
	\item[(1)$\Rightarrow$(2)] Let (f \in K[x]) be irreducible, and let (\mu \in K[x]i) such that (f(\mu) = \mu). Suppose (\alpha \in F) is a root of (f). There exists a field homomorphism [ \sigma: K(x) \to F ] such that (o(\alpha) = \beta), where (o) is the norm (since (o|K = \text{id.})) or it extends to $[ o: F^* \to G{L/K} ]$ with $[ o(F) = F_K ]$ due to the normality of (F) over (K). Consequently, the roots of (f) lie in (F), and thus $(p|{F_K} = s_n)$.
\end{itemize}